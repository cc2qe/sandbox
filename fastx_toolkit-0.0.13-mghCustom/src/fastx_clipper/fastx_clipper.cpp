...
"Modified by C. Chiang and A. Heilbut for CHGR\n" \
"\n" \
...
int main(int argc, char* argv[])
{
	...
	while ( fastx_read_next_record(&fastx) ) {
		...
		if (i==0 || i==1) { // empty sequence or single base ? (in which the adapter was found at index 0 or 1)
			// If single base must change qual to 0, since GATK complains about single bases with quality '*'
			// count_discarded_adapter_at_index_zero += reads_count;
			i += keep_delta;
			
			//Change read to single base 'N', change quality to 0
			fastx.nucleotides[0] = 'N' ;
			fastx.quality[0] = 2 ;
			fastx.nucleotides[1] = 0 ;
	...
	//		std::cout << "discard clipped" << std::endl;
			continue;
		}
