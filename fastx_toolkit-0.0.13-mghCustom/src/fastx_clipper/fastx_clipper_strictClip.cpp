/*
    ...
    Modified by Colby Chiang so that adapter-only reads won't
    cause orphaned pairs (10/4/2010).
...
"Modified by Colby Chiang, 10/4/10\n"
"\n" \
...
int adapter_cutoff_index ( const SequenceAlignmentResults& alignment_results )
{
	...
	// maximum percent of mismatches over the extended alignment size
	int max_mismatch_pct = 25 ;
	...
	// CC: While the above comment is theoretically true, I am not going to add
	// mismatches for missing from query end because base quality scores often
	// drop off.
	...
	//missing_from_end = std::min(missing_from_query_end, missing_from_target_end);
	
	mismatches += missing_from_end ;
	int extended_size = alignment_size +
		alignment_results.target_start +
		missing_from_end ;
...
//		  << std::endl;
	...
	if (mismatches * 100 / extended_size >= max_mismatch_pct)
		return -1;
