...
"Modified by Colby Chiang, 10/26/10\n" \
"Prevents orphaning of paired ends.\n" \
"\n" \
...
int adapter_cutoff_index ( const SequenceAlignmentResults& alignment_results )
{
	...
	"Missing from start = " << alignment_results.target_start
...
int main(int argc, char* argv[])
{
	...
	while ( fastx_read_next_record(&fastx) ) {
		...
		if (i==0 || i==1) { // empty sequence or single base ? (in which the adapter was found at index 0 or 1)
			// If single bask must change qual to 0, since GATK complains about single bases with quality '*'
