...
def getFrequency(inFile, numIter, minDist, tag, yates, sampleFile):
    ...
    count = 0
    while count < numIter:
        # combo string to store the output
        combo_str = list()
        ...
        if (snp1[0]!=snp2[0] or abs(float(snp1[1])-float(snp2[1]))>=minDist) and (snp1[0]!=snp3[0] or abs(float(snp1[1])-float(snp3[1]))>=minDist) and (snp2[0]!=snp3[0] or abs(float(snp2[1])-float(snp3[1]))>=minDist):
            ...
            for c in combos:
                ...
                for subpop in subpop_list:
                    ...
                    subpop_obsMinusExp[subpop] = subpop_obs[subpop] - subpop_exp[subpop]
                ...
                amr_str = '%.2f|%.2f|%.2f' % (subpop_obs['amr'], subpop_exp['amr'], subpop_obsMinusExp['amr'])
                afr_str = '%.2f|%.2f|%.2f' % (subpop_obs['afr'], subpop_exp['afr'], subpop_obsMinusExp['afr'])
                asn_str = '%.2f|%.2f|%.2f' % (subpop_obs['asn'], subpop_exp['asn'], subpop_obsMinusExp['asn'])
                eur_str = '%.2f|%.2f|%.2f' % (subpop_obs['eur'], subpop_exp['eur'], subpop_obsMinusExp['eur'])
                total_str = '%.2f|%.2f|%.2f' % (obs, exp, obsMinusExp)

                amr_strNum = '%.2f|%.2f|%.2f' % (subpop_obsNum['amr'], subpop_expNum['amr'], subpop_obsMinusExp['amr'] * len(subpop_set['amr']))
                afr_strNum = '%.2f|%.2f|%.2f' % (subpop_obsNum['afr'], subpop_expNum['afr'], subpop_obsMinusExp['afr'] * len(subpop_set['afr']))
                asn_strNum = '%.2f|%.2f|%.2f' % (subpop_obsNum['asn'], subpop_expNum['asn'], subpop_obsMinusExp['asn'] * len(subpop_set['asn']))
                eur_strNum = '%.2f|%.2f|%.2f' % (subpop_obsNum['eur'], subpop_expNum['eur'], subpop_obsMinusExp['eur'] * len(subpop_set['eur']))
                total_strNum = '%.2f|%.2f|%.2f' % (obsNum, expNum, obsNum - expNum)

                # combo_str.append(map(str, ['COMBO', id, snp1[0], snp1[1], snp1[2], round(af1[0],2), round(af1[1],2), round(af1[2],2), snp2[0], snp2[1], snp2[2], round(af2[0],2), round(af2[1],2), round(af2[2],2), snp3[0], snp3[1], snp3[2], round(af3[0],2), round(af3[1],2), round(af3[2],2), c2, amr_str, afr_str, asn_str, eur_str, total_str, round(subpop_chi['amr'],2), round(subpop_chi['afr'],2), round(subpop_chi['asn'],2), round(subpop_chi['eur'],2), round(chi,2)]))
                combo_str.append(map(str, ['COMBO', id, snp1[0], snp1[1], snp1[2], round(af1[0],2), round(af1[1],2), round(af1[2],2), snp2[0], snp2[1], snp2[2], round(af2[0],2), round(af2[1],2), round(af2[2],2), snp3[0], snp3[1], snp3[2], round(af3[0],2), round(af3[1],2), round(af3[2],2), c2, amr_str, afr_str, asn_str, eur_str, total_str, amr_strNum, afr_strNum, asn_strNum, eur_strNum, total_strNum, round(subpop_chi['amr'],2), round(subpop_chi['afr'],2), round(subpop_chi['asn'],2), round(subpop_chi['eur'],2), round(chi,2)]))

            for s in combo_str:
                print '\t'.join(map(str, (s + [round(subpop_pSum['amr'],2), round(subpop_pSum['afr'],2), round(subpop_pSum['a\
sn'],2), round(subpop_pSum['eur'],2), round(pSum,2)]) ))
