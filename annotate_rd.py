...
description: Compute genotype of structural variants based on breakpoint depth")
    ...
    parser.add_argument('-r', '--root', type=str, required=True, help='CNVnator .root histogram file')
...
# primary function
def sv_genotype(vcf_file, sample, root, vcf_out, debug):
    in_header = True
    header = []
    vcf = Vcf()
    ...
    p1 = Popen(['cat', coord_list.name], stdout=PIPE, stderr=STDOUT)
    cmd = map(str, ['cnvnator', '-root', root, '-genotype', window])
    p2 = Popen(cmd, stdin=p1.stdout, stdout=PIPE, stderr=STDOUT)
    p3 = Popen(['awk', '{ print $4 }'], stdin=p2.stdout, stdout=PIPE, stderr=STDOUT)
...
def main():
    ...
    sv_genotype(args.input_vcf, args.sample, args.root, args.output_vcf, args.debug)
