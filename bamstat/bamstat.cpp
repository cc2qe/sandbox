...
int writeHTMLReport(std::string filename) {
    std::ofstream reportfile(filename.c_str());

    reportfile << "<html>";
    reportfile << "<title>BAM Alignment Statistics</title>";

    reportfile << "<h2>Filename: </h2>";

// <h5>Insert Size Distribution: F-R (normal, short-insert) read pairs</h5>

// <img src="isize_short.png" width=600/>

// <h5>Insert Size Distribution: R-F (jumping long-insert) read pairs</h5>

// <img src="isize_long.png" width=600/>
...
int main(int argc, char **argv) {
 ...
 while (sampleCount < 1000000 && samread(samfile, rec1) > 0) { 
    ...
    if (strncmp(bam1_qname(rec1), bam1_qname(rec2), strlen(bam1_qname(rec1)-2)) != 0) {
   	...
   	printf("Please sort the input alignment by the read ID.\n");
    return 0;
   }
...
//		std::cout << rec1->core.isize << "\t" << rec1->core.pos << "\t" << rec2->core.pos << "\t" << bam1_qname(rec1) << std::endl;
 ...
 while (samread(samfile, rec1) > 0) { 
   ...
   if (testFlag(rec1->core.flag, BAM_FPROPER_PAIR)) {
    ...
    // the R/F or F/R orientation is dependent on the mapped genomic location - the actual sequencing fragment could 
